...
async def create_faculties_state():
    ...
    for i in range(len(faculties)):
        if faculties[i]['inactive'] is False:
            faculties_ary_state.append(
                {
                    "short_name": faculties[i]["short_name"],
                    "is_selected": False,
                    "id": str(faculties[i]["id"])
                }
            )
...
async def create_faculties_keyboard(faculties_ary_state):
    ...
    for i in range(len(faculties_ary_state)):
        if faculties_ary_state[i]["is_selected"]:
            faculties_keyboard.add(InlineKeyboardButton(
                text='✅' + faculties_ary_state[i]["short_name"],
            ...
            faculties_keyboard.add(InlineKeyboardButton(
                text=str(faculties_ary_state[i]["short_name"])[1:],
                callback_data="fac_" + faculties_ary_state[i]["id"]
            ))
        else:
            faculties_keyboard.add(InlineKeyboardButton(
    ...
    faculties_keyboard.row(
        InlineKeyboardButton(
            text=BACK_BTN,
            callback_data='back_to_select_lesson_num'
        ),
        InlineKeyboardButton(
            text=CONTINUE_BTN,
