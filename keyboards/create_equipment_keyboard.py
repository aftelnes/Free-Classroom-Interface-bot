...
async def create_equipments_keyboard(equipments_ary_state):
    ...
    for i in range(len(equipments_ary_state)):
        if equipments_ary_state[i]["is_selected"]:
            equipments_keyboard.add(InlineKeyboardButton(
                text='✅' + equipments_ary_state[i]["name"],
            ...
            equipments_keyboard.add(InlineKeyboardButton(
                text=str(equipments_ary_state[i]["name"])[1:],
                callback_data="equip_" + equipments_ary_state[i]["id"]
            ))
        else:
            equipments_keyboard.add(InlineKeyboardButton(
    ...
    equipments_keyboard.row(
        InlineKeyboardButton(
            text=BACK_BTN,
            callback_data='select_faculties'
        ),
        InlineKeyboardButton(
            text=CONTINUE_BTN,
            callback_data='select_size'
        )
    )
