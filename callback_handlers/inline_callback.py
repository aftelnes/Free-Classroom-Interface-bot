...
#TODO Такой вопрос, в примере, который ты мне скинул нет такой логике обработки колюэков от инлайн кнопок
#TODO Это довольно таки массивно и громоздко в правке и чтении, можешь описать какая у тебя используется архитектура?

@callback_handlers_router.message(CommandStart())
async def start_cmd(message: types.Message, state: FSMContext):
    '''По команде /start показывает клавиатуру с кнопкой "Выбрать дату"'''

    #TODO Инициализируем пустые поля в сторе FSMContext, без инициализации ругается, что добавляет обновление в несуществующее поле
...
async def callback_calendar_keyboard(callback_query: CallbackQuery, callback_data: CallbackData, state: FSMContext):
    ...
    if selected:
        ...
        print('date_from_user = ', data["date_from_user"])
...
async def show_lesson_num_keyboard(callback_querry: CallbackQuery):
    ...
    await callback_querry.message.edit_reply_markup(reply_markup=await create_lesson_num_keyboard())
...
async def show_faculties_keyboard(callback_querry: CallbackQuery, state: FSMContext):
    ...
    await callback_querry.message.edit_text(text=message_text, reply_markup=await create_faculties_keyboard())
...
async def callback_faculties_keyboard(callback_querry: CallbackQuery, state: FSMContext):
    ...
    updated_faculties.append(int(callback_querry.data[4:]))
...
async def show_faculties_keyboard(callback_querry: CallbackQuery):
    ...
    await callback_querry.message.edit_reply_markup(reply_markup=await create_faculties_keyboard())
...
async def show_equipment_keyboard(callback_querry: CallbackQuery, state: FSMContext):
    ...
    await callback_querry.message.edit_text(text=message_text, reply_markup=await create_equipment_keyboard())
...
async def show_equipment_keyboard(callback_querry: CallbackQuery):
    ...
    await callback_querry.message.edit_reply_markup(reply_markup=await create_equipment_keyboard())




@callback_handlers_router.callback_query(F.data == 'select_size')
async def show_size_keyboard(callback_querry: CallbackQuery, state: FSMContext):
        '''Фнукия отправляет клавиатуру с выбором кол-ва необходимых посадочных мест'''
        data = await state.get_data()
        message_text = f'Дата: {data["date_from_user"]}\nПара: {data["lesson_num"]}\nФакультеты: {data["faculties_short_name"]}\nОснащение: {data["equipments_name"]}'
        await callback_querry.message.edit_text(text=message_text, reply_markup=select_size)
...
async def show_find_keyboard(callback_querry: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    result = get_free_classrooms(data["date_from_user"], data["lesson_num"], data["faculties"], data["equipments"], data["size"])
